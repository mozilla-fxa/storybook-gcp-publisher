...
const configSchema = {
  ...
  github: {
    repo: {
      ...
      format: String,
      env: 'STORYBOOKS_GITHUB_REPO',
      default: null,
    },
    mainBranch: {
      doc: 'main branch for git repo',
      format: String,
      env: 'STORYBOOKS_GITHUB_MAIN_BRANCH',
      default: 'main',
    },
    token: {
...
async function setupConfigOptions(program) {
  program
    ...
    .option('--commit-description <file>', 'read commit description from file')
    ...
    .option('--no-skip-on-bad-config', 'skip build on missing configuration, rather than report error');
}
...
async function loadConfig(program) {
  ...
  try {
    ...
    if (program.skipOnBadConfig) {
      console.log('invalid configuration, skipping build', err);
      process.exit(0);
    } else {
      throw err;
    }
